...
class TransactionCard extends ConsumerWidget {
  ...
  const TransactionCard({required this.transaction, this.onTap, Key? key})
    : super(key: key);
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final aims = ref.watch(aimProvider).value ?? [];
    ...
    return GestureDetector(
      onTap: () async {
        if (isValidPayment) {
          ...
          context.push(
            '${context.sourceRoutePath}/migration',
            extra: MigrationData(
                    ...
                    child: Column(
                      ...
                      children: <Widget>[
                        if (transaction.importDeadline != null)
                          ItemRow(
                            t1: 'backupExpire'.tr(),
                            t2: MigrationExportScreen.format.format(
                              transaction.importDeadline!,
                            ),
                          ),
  ...
  Color iconColor(TransactionType type) {
    return switch (type) {
      TransactionType.VOUCHERS ||
      TransactionType.MIGRATION_IMPORT ||
      TransactionType.EXCHANGE_IMPORT => Colors.green,
      TransactionType.PAYMENT ||
      TransactionType.MIGRATION_EXPORT ||
      TransactionType.EXCHANGE_EXPORT => Colors.red,
    };
  }
}

enum CardAction { migration, payment, exchange, share }
